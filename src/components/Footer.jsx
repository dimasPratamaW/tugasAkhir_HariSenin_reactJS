...
const Footer = () => {
  return (
    <footer className="mt-20 w-full h-max bg-gray-900 font-body font-normal">
      ...
      <div className="w-full flex flex-wrap md:flex-nowrap justify-center items-start gap-5 py-5 px-5">
          ...
          {dataFooter.map((elm, i) => {
            return (
              <div className="pb-4 md:max-w-[400px]" key={i}>
                <p className="md:text-2xl font-medium text-gray-200 pb-3">{elm.label}</p>
                <p className="text-gray-300 pb-2 text-xs cursor-pointer hover:text-gray-200">{elm.title}</p>
                <ul className="grid gap-2">
                  {elm.sub.map((label, i) => {
                    return (
                      <li key={i} className="cursor-pointer text-xs md:text-base text-gray-400 hover:text-gray-300">
                        {label}
                      </li>
                    );
                  })}
                  <p className="text-gray-300 text-xs md:text-base hover:text-gray-200 cursor-pointer">{elm.Privasi}</p>
                </ul>
              </div>
            );
          })}
        ...
        <div className="grid w-full text-gray-300">
          <div className="flex items-center text-gray-300 gap-2 pb-10 border-b-[1px] w-full border-gray-600">
            ...
            <span className="text-sm md:text-base">Bahasa Indonesia</span>
          </div>
          <p className="pt-8 text-xs md:text-base">Odoo adalah rangkaian Aplikasi bisnis open source yang mencakup semua kebutuhan perusahaan Anda : CRM, eCommerce, akutansi, iventaris, point of sale, manajement project, dan seterus.</p>
          <span className="pt-3 text-xs md:text-base">Mudah di gunakan dan terintegrasi penuh pada saat yang sama adalah value proposition unik Odoo.</span>
